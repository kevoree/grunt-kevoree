...
module.exports = function (grunt, options, done) {
  ...
  function goodToGo(already) {
    ...
    clog(grunt)(chalk.green('✔') + ' ' + chalk.cyan('kevoree-cli') + '@' + chalk.yellow(localPkg.version) + (already ? ' already':'') + ' installed ' + chalk.gray('('+(end - start)+'ms)'));
    done();
  }

  function doInstall() {
    var cmd = 'npm';
    var args = [ 'install', 'kevoree-cli@' + options.runtime ];
