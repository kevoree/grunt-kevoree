...
module.exports = function (grunt) {
  ...
  grunt.registerTask('kevoree', 'Automatically runs kevoree runtime (works like mvn kev:run plugin)', function () {
    ...
    var realModulesPath = path.resolve(options.modulesPath, options.node);
    // write a fake package.json to make the whole thing work
    mkdirp(realModulesPath, function (err) {
      if (err) {
        grunt.fail.fatal(err);
      } else {
        fs.writeFile(path.join(realModulesPath, 'package.json'),
          JSON.stringify({
            name: 'kevoree-local-dus',
            version: '0.0.0',
            private: true,
            description: 'Auto-generated by grunt-kevoree'
          }, null, 2),
          'utf8',
          function (err) {
            if (err) {
              ...
              var npmResolver = new NPMResolver(realModulesPath, logger),
              ...
              try {
                ...
                try {
                  var dirs = fs.readdirSync(path.join(realModulesPath, 'node_modules'));
                  dirs.forEach(function (dir) {
                    try {
                      var kevlibStr = fs.readFileSync(path.resolve(realModulesPath, 'node_modules', dir, 'kevlib.json'), {
                        encoding: 'utf8'
                      });
                ...
                var mergeTasks = options.mergeLocalLibraries.map(function (localLibPath) {
                  return function (cb) {
                    ...
                    npmLink(localLibPkg.name, localLibPath, realModulesPath, function (err) {
                      if (err) {
                        cb(new Error('"grunt-kevoree" unable to merge local library ' + localLibPkg.name + ' in ' + realModulesPath));
                      } else {
                    ...
                    kevsEngine.parse(kevscriptContent, contextModel, options.ctxVars, function (err, model) {
                      if (err) {
                        ...
                        var linkModule = function () {
                          ...
                          if (!grunt.file.exists(realModulesPath)) {
                            grunt.log.ok('Linking ' + pkg.name + ' in ' + path.relative(process.cwd(), realModulesPath) + ' ...');
                            npmLink(pkg.name, process.cwd(), realModulesPath, function (err) {
                              if (err) {
                                grunt.fail.fatal('"grunt-kevoree" unable to link ' + pkg.name + ' in ' + realModulesPath);
                                done();
                              } else {
                                startRuntime();
                              }
                        ...
                        var startRuntime = function () {
                          ...
                          var Kevoree = require('kevoree-nodejs-runtime'),
                            runtime = new Kevoree(realModulesPath, logger, npmResolver);
